...
CI 会在针对 `main` / `develop` 的 PR 上自动运行（见 `.github/workflows/ci.yml`）；合并后 push 到 `main` / `develop` 也会再跑一遍。`build-and-test`（单元测）、`integration-tests`（Host 集成测）与 `e2e-tests`（Playwright 浏览器 E2E）均须通过。
